...
export default function Terms({ sheet, onClose }: { sheet: Extract<ClientSheet, { kind: "terms" }>; onClose: () => void }) {
  ...
  const sub = [version?.version !== null && version?.version !== undefined ? `v${version.version}` : null, since === null ? null : t("client.sheet.inForceSince", { date: dayLabel(since, locale, "long") })]
    .filter((x) => x !== null)
    .join(" · ");
  return (
